...
class MuseTalkWorker:
    ...
    def infer_streaming(
        ...
        """Streaming inference. Yields (BGR numpy array, fps) per frame.

        Same pipeline as ``infer`` but skips disk I/O and ffmpeg encoding —
        frames are returned directly for real-time WebSocket delivery.
        """
        ...
        with self._lock, torch.no_grad():
            ...
            for whisper_batch, latent_batch in gen:
                ...
                for res_frame in recon:
                    ...
                    ori_frame = _copy.deepcopy(frame_list_cycle[frame_idx % len(frame_list_cycle)])
                    x1, y1, x2, y2 = bbox
                    if self.version == "v15":
                        y2 = min(y2 + extra_margin, ori_frame.shape[0])
                    try:
